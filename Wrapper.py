...
class HV_Wrapper():
    def __init__(self):
        ...
        [
            ...
            {'cmd': 'set_v',            'args':'snmpset -OqvU -v 2c -m +WIENER-CRATE-MIB -c guru {ip} outputVoltage.u{channel} F {value}'},
            ...
            {'cmd': 'set_min_v',        'args':'snmpset -OqvU -v 2c -m +WIENER-CRATE-MIB -c guru {ip} outputSupervisionMinSenseVoltage.u{channel} F {value}'},
            {'cmd': 'get_max_v',        'args':'snmpget -OqvU -v 2c -m +WIENER-CRATE-MIB -c guru {ip} outputSupervisionMaxSenseVoltage.u{channel}'},
            {'cmd': 'set_max_v',        'args':'snmpset -OqvU -v 2c -m +WIENER-CRATE-MIB -c guru {ip} outputSupervisionMaxSenseVoltage.u{channel} F {value}'},
            
            # Current SNMP commands
            {'cmd': 'get_i',            'args':'snmpget -OqvU -v 2c -m +WIENER-CRATE-MIB -c guru {ip} outputMeasurementCurrent.u{channel}'},
            {'cmd': 'set_i',            'args':'snmpset -OqvU -v 2c -m +WIENER-CRATE-MIB -c guru {ip} outputCurrent.u{channel} F {value}'},
            ...
            {'cmd': 'set_max_i',        'args':'snmpset -OqvU -v 2c -m +WIENER-CRATE-MIB -c guru {ip} outputSupervisionMaxCurrent.u{channel} F {value}'},

            # (Voltage) Rise/Fall Rate SNMP commands
            ### Not sure how useful these will be
            {'cmd': 'set_v_rise',       'args':'snmpset -OqvU -v 2c -m +WIENER-CRATE-MIB -c guru {ip} outputVoltageRiseRate.u{channel} F {value}'},
            {'cmd': 'get_set_v_rise',   'args':'snmpget -OqvU -v 2c -m +WIENER-CRATE-MIB -c guru {ip} outputVoltageRiseRate.u{channel}'},
            
            {'cmd': 'set_v_fall',       'args':'snmpset -OqvU -v 2c -m +WIENER-CRATE-MIB -c guru {ip} outputVoltageFallRate.u{channel} F {value}'},
            ...
            {'cmd': 'set_i_rise',       'args':'snmpset -OqvU -v 2c -m +WIENER-CRATE-MIB -c guru {ip} outputCurrentRiseRate.u{channel} F {value}'},
            {'cmd': 'get_set_i_rise',   'args':'snmpget -OqvU -v 2c -m +WIENER-CRATE-MIB -c guru {ip} outputCurrentRiseRate.u{channel}'},
            
            {'cmd': 'set_i_fall',       'args':'snmpset -OqvU -v 2c -m +WIENER-CRATE-MIB -c guru {ip} outputCurrentFallRate.u{channel} F {value}'},
